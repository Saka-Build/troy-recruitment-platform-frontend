...
const filteredSubmissions = useMemo(() => {
    ...
    const groupedData = useMemo(() => {
        ...
        filteredSubmissions.forEach((item) => {
            const clientName = item.clientName?.trim() || "No Client";
            const endClientName = item.endClientName?.trim() || "No End Client";
            const jobName = item.jobName?.trim() || "Unnamed Job";
            ...
            groups[clientName].endClients[endClientName].jobs[jobName].submissions.push(item);});
        ...
        if (sortConfig.field === "client") {
            clientsArray.sort((a, b) =>a.name.localeCompare(b.name) * sortDirection);
        } else {
            clientsArray.sort((a, b) =>a.name.localeCompare(b.name));
        }

        clientsArray.forEach((client) => {
            if (sortConfig.field === "endClient") {
                client.endClients.sort((a, b) =>a.name.localeCompare(b.name) * sortDirection);
            } else {
                client.endClients.sort((a, b) =>a.name.localeCompare(b.name));
            }

            client.endClients.forEach((endClient) => {
                if (sortConfig.field === "job"
                ) {
                    endClient.jobs.sort((a, b) =>    a.name.localeCompare(b.name) * sortDirection);
                } else {
                    endClient.jobs.sort((a, b) =>    a.name.localeCompare(b.name));
                }
            });
        });
        return clientsArray;
    ...
    const expandAll = () => {
        ...
        groupedData.forEach((client) => {
            ...
            client.endClients.forEach((endClient) => {
                const endKey = `${client.name}__${endClient.name}`;
                endClientState[endKey] = true;

                endClient.jobs.forEach((job) => {
                    const jobKey = `${client.name}__${endClient.name}__${job.name}`;
    ...
    const getSortIcon = (field) => {
        if (sortConfig.field !== field) {
            return (<FiChevronDown className="sort-icon-neutral" />);
        }
        return (
            <FiChevronDown className={ sortConfig.direction === "asc"? "sort-icon-asc": "sort-icon-desc"} />
        );
    };
            ...
            (row, rowNumber) => {
                ...
                row.eachCell(
                    (cell,columnNumber) => {
                        ...
                        if (columnNumber === 10) {cell.numFmt = "@";}
                        cell.alignment = {
                            vertical: "middle",
                            horizontal:
                                columnNumber === 5 ||
...
<div className="report-multiselect">
            ...
            {applicationStatusFilter.length === 0
                ...
                : applicationStatusFilter.length === 1
                    ? applicationStatuses.find(
                        (status) =>
                            status.value ===
                            applicationStatusFilter[0]
                    )?.label
                    : `${applicationStatusFilter.length} statuses selected`}
        </span>
    ...
    {showStatusDropdown && (
        <div className="report-multiselect-menu">
            ...
            <label className="report-multiselect-option">
                <input
                    type="checkbox"
                    checked={
                        applicationStatusFilter.length === 0
            ...
            {applicationStatuses.map(
                (status) => (
                    <label
                        key={status.value}
                        className="report-multiselect-option"
                    >
                        <input
                            type="checkbox"
                            checked={applicationStatusFilter.includes(
                                status.value
                            )}
                            onChange={() =>
                        ...
                        <table className="reports-table">
                                ...
                                <tr>
                                    ...
                                    <th>
                                        <button
                                            type="button"
                                            className="report-sort-header"
                                            onClick={() =>handleSort(    "endClient")}
                                        >
                                            <span>End Client</span>
                                            {getSortIcon("endClient")}
                                        </button>
                                    </th>
                                    <th>
                                        <button
                            ...
                            <tbody>
                                {groupedData.length > 0 ? (
                                    groupedData.map(
                                        (client) => {
                                            const clientExpanded = expandedClients[client.name];
                                            return (
                                                <React.Fragment key={client.name}>
                                                    ...
                                                    >
                                                        <td colSpan="9">
                                                            <div className="report-group-row-content">
                                                                ...
                                                                <span className="report-group-label report-group-client-label"> {client.name}</span>

                                                                <span className="report-group-count">
                                                                    {
                                                                        client.endClients.reduce(
                                                                            ( total, endClient) => total + endClient.jobs.reduce((jobTotal,job) =>jobTotal +job.submissions.length, 0 ),0
                                                                        )
                                                                    }{" "}
                                                                    submissions
                                                                </span>
                                                    ...
                                                    {clientExpanded &&client.endClients.map( (endClient) => {
                                                                const endKey =`${client.name}__${endClient.name}`;
                                                                const endExpanded =expandedEndClients[endKey];

                                                                const endClientCount =
                                                                    endClient.jobs.reduce(
                                                                        ( total, job) => total + job.submissions.length,0
                                                                    );

                                                                return (
                                                                    <React.Fragment key={  endKey } >
                                                                        <tr
    ...
    {item.candidateId ? (
        ...
        <span className="report-cv-id">
            {displayValue(item.candidateCVId)}
                                                                                                                    ...
                                                                                                                    <div className="report-candidate-info">
                                                                                                                        ...
                                                                                                                        <div className="report-candidate-contact">
                                                                                                                            ...
                                                                                                                            {item.candidateEmail ? (
                                                                                                                                ...
                                                                                                                                <span className="report-candidate-contact-icon report-candidate-contact-disabled"><FiMail /> </span>
                                                                                                                            )}
                                                                                                                            {item.candidatePhone ? (
                                                                                                                                <a
                                                                                                                                ...
                                                                                                                                <span className="report-candidate-contact-icon report-candidate-contact-disabled"> <FiPhone /> </span>
                                                                                                                            )}
                                    ...
                                    <tr>
                                        <td colSpan="9" className="empty-report">
                                            <div className="empty-report-content">
                                                <div className="empty-report-icon"><FiSearch /></div>
